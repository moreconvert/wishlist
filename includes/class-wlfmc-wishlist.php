...
if ( ! class_exists( 'WLFMC_Wishlist' ) ) {
	...
	class WLFMC_Wishlist extends WC_Data implements ArrayAccess {
		...
		public function is_current_user_owner( $current_user = false ) {
			...
			if ( $this->is_session_based() && WLFMC_Session()->get_session_id() == $session_id ) {
				return true;
			}
		...
		public function get_url() {
			if ( is_user_logged_in() ) {
				return wc_get_account_endpoint_url( 'wlfmc-wishlist' );
			} else {
				return WLFMC()->get_wishlist_url( 'view/' . $this->get_token() );
			}
		}
		 ...
		 * Set session id
		 *
		 * @param int $session_id Sessionid.
		 */
		...
		public function get_total() {
			$total = 0;
			foreach ( $this->get_items() as $product_id => $item ) {
