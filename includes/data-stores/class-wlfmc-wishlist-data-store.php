...
if ( ! class_exists( 'WLFMC_Wishlist_Data_Store' ) ) {
	...
	class WLFMC_Wishlist_Data_Store {
		...
		public function create( &$wishlist ) {
			...
			if ( ! $wishlist->get_session_id() && ! $wishlist->get_user_id() ) {
				if ( is_user_logged_in() ) {
					...
					$wishlist->set_user_id( apply_filters( 'wlfmc_add_wishlist_user_id', $user_id ) );
				} else {
					$session_id = WLFMC_Session()->get_session_id();
					$wishlist->set_session_id( apply_filters( 'wlfmc_add_wishlist_session_id', $session_id ) );
				}
			}
			...
			if ( $res ) {
				$id = apply_filters( 'wlfmc_wishlist_correctly_created', intval( $wpdb->insert_id ) );
		...
		public function read( &$wishlist ) {
			...
			if ( ! $id && ! $token ) {
				throw new Exception( __( 'Invalid wishlist.', 'wc-wlfmc-wishlist' ) );
			}
		...
		public function delete( &$wishlist ) {
			...
			$wpdb->delete( $wpdb->wlfmc_items, array( 'wishlist_id' => $id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		...
		public function delete_expired() {
			global $wpdb;
			$wpdb->query( "DELETE FROM {$wpdb->wlfmc_items} WHERE wishlist_id IN ( SELECT ID FROM {$wpdb->wlfmc_wishlists} WHERE expiration < NOW() and user_id IS NULL )" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		...
		public function query( $args = array() ) {
			...
			if ( empty( $show_empty ) ) {
				$sql .= " AND l.`ID` IN ( SELECT wishlist_id FROM {$wpdb->wlfmc_items} )";
			}
			...
			if ( ! empty( $lists ) ) {
				$lists = array_map( array( 'WLFMC_Wishlist_Factory', 'get_wishlist' ), $lists );
			} else {
				$lists = array();
			}
		 ...
		 * @param array $column_values Array of values to apply to the query; must have same number of elements of columns, and they must respect defined type.
		...
		public function read_items( $wishlist ) {
			...
			if ( false === $items ) {
				$query = "SELECT i.* FROM {$wpdb->wlfmc_items} as i INNER JOIN {$wpdb->posts} as p on i.prod_id = p.ID WHERE wishlist_id = %d AND p.post_type IN ( %s, %s ) AND p.post_status = %s";

				// remove hidden products from result.
				$hidden_products = wlfmc_get_hidden_products();

				if ( ! empty( $hidden_products ) && apply_filters( 'wlfmc_remove_hidden_products_via_query', true ) ) {
		...
		public function delete_items( $wishlist ) {
			...
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->wlfmc_items} WHERE wishlist_id = %d", $wishlist->get_id() ) ); // phpcs:ignore WordPress.DB
		...
		public function generate_token() {
			...
			$sql = "SELECT COUNT(*) FROM `{$wpdb->wlfmc_wishlists}` WHERE `wishlist_token` = %s";
		...
		public function assign_to_user( $session_id, $user_id ) {
			...
			// update any item that is assigned to the .
			$items = $wpdb->get_col( $wpdb->prepare( "SELECT i.ID FROM {$wpdb->wlfmc_items} AS i LEFT JOIN {$wpdb->wlfmc_wishlists} AS l ON l.ID = i.wishlist_id WHERE l.session_id = %s", $session_id ) ); // phpcs:ignore WordPress.DB

			if ( ! empty( $items ) ) {
				$items_string = implode( ',', array_map( 'esc_sql', $items ) );
				$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->wlfmc_items} SET user_id = %d WHERE ID IN ({$items_string})", $user_id ) ); // phpcs:ignore WordPress.DB
			}
			...
			$default_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->wlfmc_wishlists} WHERE is_default = %d AND user_id = %d ORDER BY dateadded ASC", 1, $user_id ) ); // phpcs:ignore WordPress.DB
			...
			if ( count( $default_ids ) > 1 ) {
				...
				try {
					...
					if ( apply_filters( 'wlfmc_merge_default_wishlists', true ) ) {
						// change wishlist id to master default id.
						$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->wlfmc_items} SET wishlist_id = %d WHERE wishlist_id IN ({$where_statement})", $master_default_wishlist ) ); // phpcs:ignore WordPress.DB

						// delete slave default wishlists.
						$wpdb->query( "DELETE FROM {$wpdb->wlfmc_wishlists} WHERE ID IN ({$where_statement})" ); // phpcs:ignore WordPress.DB
					} else {
						// otherwise, we just leave all the wishlists as they are, but we remove default flag from latest.
						// remove default flag.
						$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->wlfmc_wishlists} SET is_default = %d WHERE ID IN ({$where_statement})", 0 ) ); // phpcs:ignore WordPress.DB

						// set name where it is missing.
						$default_title = apply_filters( 'wlfmc_default_wishlist_formatted_title', __( 'My Wishlist', 'wc-wlfmc-wishlist' ) );
						$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->wlfmc_wishlists} SET wishlist_name = %s WHERE ID IN ({$where_statement}) AND wishlist_name = ''", $default_title ) ); // phpcs:ignore WordPress.DB
					}
				} catch ( Exception $e ) {
					return;
				}
		...
		public function slug_exists( $slug ) {
			...
			$res = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(ID) FROM {$wpdb->wlfmc_wishlists} WHERE wishlist_slug = %s", $slug ) ); // phpcs:ignore WordPress.DB
		...
		protected function should_be_default() {
			global $wpdb;
			$user_id     = get_current_user_id();
			$customer_id = WLFMC_Session()->maybe_get_session_id();

			if ( $user_id ) {
				$wishlists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( ID ) FROM {$wpdb->wlfmc_wishlists} WHERE user_id = %d AND is_default = %d", $user_id, 1 ) ); // phpcs:ignore WordPress.DB

				return ! (bool) $wishlists;
			}

			if ( $customer_id ) {
				$wishlists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( ID ) FROM {$wpdb->wlfmc_wishlists} WHERE session_id = %s AND expiration > NOW() AND is_default = %d", $customer_id, 1 ) ); // phpcs:ignore WordPress.DB
		...
		protected function clear_caches( &$wishlist ) {
			...
			if ( $user_id ) {
				wp_cache_delete( 'user-wishlists-' . $user_id, 'wlfmc-wishlists' );
			}

			if ( $session_id ) {
				wp_cache_delete( 'user-wishlists-' . $session_id, 'wlfmc-wishlists' );
			}
		}
	}
}
