	...
	class WLFMC_Wishlist_Item_Data_Store {
		...
		public function create( &$item ) {
			...
			$values  = array(
				...
				apply_filters( 'wlfmc_adding_to_wishlist_original_price', $item->get_product_price() ),
			...
			$query         = "INSERT INTO {$wpdb->wlfmc_items} ( {$query_columns} ) VALUES ( {$query_values} ) ";
		...
		public function read( &$item ) {
			...
			if ( false === $data ) {
				$data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->wlfmc_items} WHERE ID = %d LIMIT 1;", $item->get_id() ) );
				wp_cache_set( 'item-' . $item->get_id(), $data, 'wlfmc-wishlist-items' );
			}

			if ( ! $data ) {
				throw new Exception( __( 'Invalid wishlist item.', 'wc-wlfmc-wishlist' ) );
			}
		...
		public function delete( &$item ) {
			...
			$wpdb->delete( $wpdb->wlfmc_items, array( 'ID' => $item->get_id() ) );
		...
		public function count_times_added( $product_id, $user = false ) {
			...
			$query = "SELECT
				      ...
				      LEFT JOIN {$wpdb->wlfmc_items} AS i USING( wishlist_id )
					  WHERE i.`prod_id` = %d";
		...
		public function update_raw( $columns, $column_values, $conditions = array(), $conditions_values = array(), $clear_caches = false ) {
			...
			if ( $clear_caches ) {
				...
				$query = "SELECT ID FROM {$wpdb->wlfmc_items} {$query_where}";
			...
			$query  = "UPDATE {$wpdb->wlfmc_items} SET {$query_columns} {$query_where}";
		...
		public static function filter_join_for_wishlist_count( $join ) {
			global $wpdb;
			$join .= " LEFT JOIN ( SELECT COUNT(*) AS wishlist_count, prod_id FROM {$wpdb->wlfmc_items} GROUP BY prod_id ) AS i ON ID = i.prod_id";
		...
		public static function filter_orderby_for_wishlist_count( $orderby ) {
			$orderby = 'i.wishlist_count ' . ( isset( $_REQUEST['order'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['order'] ) ) : 'ASC' );
