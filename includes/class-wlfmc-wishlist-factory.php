...
if ( ! class_exists( 'WLFMC_Wishlist_Factory' ) ) {
	...
	class WLFMC_Wishlist_Factory {
		...
		public static function get_current_wishlist( $args = array() ) {
			...
			if (
				empty( $action ) ||
				! in_array( $action, WLFMC()->get_available_views() ) ||
				in_array( $action, array( 'view', 'user' ) ) ||
				( in_array( $action, array( 'manage', 'create' ) ) && ! WLFMC()->is_multi_wishlist_enabled() )
			) {
				switch ( $action ) {
					case 'user':
						$user_id = $value;
		 ...
		 *
		 * @param $item_id int|WLFMC_Wishlist_Item|stdClass Item identifier, or item itself
		 *
		...
		public static function get_times_added_count( $product_id ) {
			try {
				$result = WC_Data_Store::load( 'wlfmc-wishlist-item' )->count_times_added( $product_id );

				return apply_filters( 'wlfmc_wishlist_times_added_count_query', $result, $product_id );
			} catch ( Exception $e ) {
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
		...
		public static function get_times_current_user_added_count( $product_id ) {
			try {
				...
				return apply_filters( 'wlfmc_wishlist_times_current_user_added_count_query', $result, $product_id );
			} catch ( Exception $e ) {
				wc_caught_exception( $e, __FUNCTION__, func_get_args() );
