...
?>
<select id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $name ); ?>[]" class="<?php echo esc_attr( $class ); ?>" multiple="multiple"
	...
	<?php if ( is_array( $options ) && ! empty( $options ) ) : ?>
		<?php foreach ( $options as $key => $val ) : ?>
			<option value="<?php echo esc_attr( $key ); ?>" <?php echo in_array( $key, $value ) ? '"selected"="selected"' : ''; ?>><?php echo esc_attr( $val ); ?></option>
		<?php endforeach; ?>
	<?php endif; ?>
</select>
<?php if ( isset( $desc ) ) : ?>
